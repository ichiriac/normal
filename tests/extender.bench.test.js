...
const { BaseHeavy, MixinHeavyNoSuper, MixinHeavyWithSuper } = require('./bench-fixtures/extendWith.classes');
...
function warmup() {
  let C;
  for (let i = 0; i < 200; i++) { C = extendWith(BaseHeavy, MixinHeavyNoSuper); }
  for (let i = 0; i < 200; i++) { C = extendWith(BaseHeavy, MixinHeavyWithSuper); }
  return C;
}
