...
describe('Request internals', () => {
    function makeQB() {
        const qb = {
            ...
            _statements: [],
            selected: null,
            select(arg) { this.selected = arg; return this; },
    ...
    test('_getRequestKey stringifies statements', () => {
        const model = { name: 'M', table: 'm', columns: ['id'] };
        const qb = makeQB();
        qb._statements.push({ grouping: 'columns', value: ['id'] }, { grouping: 'where', value: { id: 1 } });
        const req = new Request(model, qb);
    ...
    test('_ensureDefaultIdSelect selects columns based on cache presence', () => {
        ...
        const qb2 = makeQB();
        const model2 = { name: 'M', table: 'm', columns: ['id', 'name'], cache: { /* truthy */ } };
    ...
    test('_wrapResult wraps rows and calls ready() when present', async () => {
        const model = {
            name: 'M',
            fields: { id: {}, name: {} },
            allocate: jest.fn((row) => ({ ...row, ready: jest.fn(async () => ({ ...row, wrapped: true })) })),
        };
        const req = new Request(model, makeQB());
        const single = await req._wrapResult({ id: 1, name: 'x' });
        expect(single.wrapped).toBe(true);

        const many = await req._wrapResult([{ id: 1, name: 'a' }, { id: 2, name: 'b' }]);
    ...
    test('_isWrappableRow edge cases', () => {
        ...
        test('then(): cache hit returns wrapped cached value without hitting QB', async () => {
            const qb = makeQB();
            qb.then = jest.fn(() => { throw new Error('QB should not be called on cache hit'); });
            const cached = [{ id: 1 }];
