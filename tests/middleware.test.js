...
const { MiddlewareSystem, DatabaseContext } = require('../src/Middleware');

describe('MiddlewareSystem', () => {
    test('executes middlewares in order and calls next once', async () => {
        const mw = new MiddlewareSystem();
        const calls = [];
        mw.use('op', async (ctx, next) => { calls.push('a'); ctx.a = 1; return next(); });
        mw.use('op', async (ctx, next) => { calls.push('b'); ctx.b = ctx.a + 1; return next(); });
        mw.use('op', async (ctx, next) => { calls.push('c'); return { done: true, ctx }; });
    ...
    test('throws if next called multiple times', async () => {
        const mw = new MiddlewareSystem();
        mw.use('op', async (ctx, next) => { await next(); return next(); });
        await expect(mw.execute('op', new DatabaseContext('op', 't'))).rejects.toThrow('next() called multiple times');
    });
    ...
    test('versioningMiddleware assigns version and updatedAt on create/update; stores previous versions', async () => {
        ...
        const updateCtx = new DatabaseContext('update', 'posts', { id: 10, data: { id: 10, title: 't2' }, previousData: { id: 10, title: 't' } });
    ...
    test('validationMiddleware validates required and type', async () => {
        const mw = new MiddlewareSystem();
        const schema = { id: { required: true, type: 'number' }, name: { required: true, type: 'string' } };
        const valMw = mw.validationMiddleware(schema);

        await expect(valMw(new DatabaseContext('create', 't', { data: { name: 'A' } }), async () => ({})))
            .rejects.toThrow(/id is required/);

        await expect(valMw(new DatabaseContext('update', 't', { data: { id: 1, name: 2 } }), async () => ({})))
            .rejects.toThrow(/name must be of type string/);

        const ok = await valMw(new DatabaseContext('create', 't', { data: { id: 1, name: 'A' } }), async () => ({ msg: 'ok' }));
    ...
    test('loggingMiddleware logs start and end; propagates errors', async () => {
        ...
        await expect(logMw(new DatabaseContext('create', 'logs', { data: { id: 2 } }), async () => { throw new Error('x'); }))
            .rejects.toThrow('x');
