...
describe('Connection config building', () => {
  test('defaults to pg config when no opts provided', () => {
    ...
    expect(c.config.connection).toMatchObject({ host: expect.any(String), port: expect.any(Number) });
  });
  ...
  test('fallback client passes through opts', () => {
    const c = new Connection({ client: 'mysql2', connection: { host: 'localhost' }, pool: { min: 0, max: 2 } });
