  ...
  loggingMiddleware() {
    return async (context, next) => {
      ...
      try {
        ...
        console.log(
          `[${new Date().toISOString()}] ${context.operation} completed in ${duration}ms`
        );
        return result;
      } catch (error) {
        const duration = Date.now() - start;
        console.error(
          `[${new Date().toISOString()}] ${context.operation} failed in ${duration}ms:`,
          error.message
        );
        throw error;
      }
...
export { MiddlewareSystem, DatabaseContext };
