...
async function processJob(job, workerId) {
  ...
  try {
    ...
    const processingTime = Date.now() - startTime;
    console.log(
      `❌ Worker ${workerId}: Failed job ${job.id} after ${processingTime}ms - ${error.message}`
    );
...
async function masterProcess() {
  ...
  cluster.on('exit', (worker, code, signal) => {
    activeWorkers--;
    console.log(
      `🔴 Worker ${worker.process.pid} died (${signal || code}). Active workers: ${activeWorkers}`
    );
...
async function workerProcess() {
  ...
  async function processNextJob() {
    try {
      ...
      const updatedRows = await workerRepo
        .get('Job')
        .query()
        .where('id', job.id)
