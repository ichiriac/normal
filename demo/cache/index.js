...
async function parentMain() {
  ...
  const fmt = (r) =>
    `${r.label}: time=${r.result.elapsedMs.toFixed(2)}ms, queries=${r.result.queries}, engine=${r.result.engine}, cache=${r.result.cacheEnabled}`;
  console.log('\nResults');
  console.log('-------');
